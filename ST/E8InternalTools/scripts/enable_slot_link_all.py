...
for DeviceNo in range (len(PciDeviceList)):
  ...
  if ( (VendorID == '0x10b5') and (DeviceID == '0x9765') ):
    ...
    if((Capabilities & 0x01600000) == 0x01600000):
      ...
      print "Enable Slot ", str(SlotNo), "Link on PLX Upstream Port", PciDeviceList[DeviceNo]

      LinkUpCommand="setpci -s " + PciDeviceList[DeviceNo] + " 78.w=0000"
#      print "LinkUpCommand    : ", LinkUpCommand

      dmesg="echo \"Enable NVMe Slot " + str(SlotNo) + " Link Upstream Port " + PciDeviceList[DeviceNo] + "\"" + " > /dev/kmsg"
