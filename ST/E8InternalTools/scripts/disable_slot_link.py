...
if (len(sys.argv) < 2):
  print "Invalid Arguments - use python disable_slot_link.py <slot number>"
  print ""
  sys.exit(1)
...
if VendorID == '8086':
  print "Disable Slot ", str(SlotNo), "Link on Intel Upstream Port", UpstreamBDF
  UpstreamLNKSTS="a2"
  UpstreamLNKCTL="a0"
else:
  print "Disable Slot ", str(SlotNo), "Link on PLX Upstream Port", UpstreamBDF
  UpstreamLNKSTS="7a"
  UpstreamLNKCTL="78"

LinkDownCommand="setpci -s " + UpstreamBDF + " " + UpstreamLNKCTL + ".w=0010"

dmesg="echo \"Disable NVMe Slot " + str(SlotNo) + " Link Upstream Port " + str(UpstreamBDF) + "\"" + " > /dev/kmsg"
