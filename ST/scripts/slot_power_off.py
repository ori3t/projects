...
if (len(sys.argv) < 2):
  print "Invalid Arguments - use python slot_power_off.py <slot number>(1..24)|all"
  print ""
  sys.exit(1)
...
# the basic syntax is  ipmitool raw 0x3c 0xae 0 bitmap1 bitmap2 bitmap3
...
for SlotNo in range (StartDrive, (EndDrive + 1)):
  dmesg="echo \"E8_slot_power_off: Disable Slot " + str(SlotNo) + " " + Node + "\"" + " > /dev/kmsg"
  subprocess.call(dmesg, shell=True)
  print "Disabling Slot:", str(SlotNo)
...
IPMICommand = "ipmitool raw 0x3c 0xae 0 " + hex(DriveMask1) + " " + hex(DriveMask2) + " " + hex(DriveMask3)
